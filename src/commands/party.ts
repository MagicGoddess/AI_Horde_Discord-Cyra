    ...
    override async autocomplete(context: AutocompleteContext): Promise<any> {
        ...
        switch(option.name) {
            ...
            case "style": {
                ...
                const ret = option.value ? available.filter(s => s.value.toLowerCase().includes(option.value.toLowerCase()) || s.name.toLowerCase().includes(option.value.toLowerCase())) : available
